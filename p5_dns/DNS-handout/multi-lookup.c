...
#define MAX_RESOLVER_THREADS 10      // Max resolver threads
...
int main(int argc, char* argv[]){
    ...
    resolver_pool = malloc(sizeof(pthread_t)*MAX_RESOLVER_THREADS);        
 
    /* Initialize the request queue, if error must exit */
    ...
    // the queue.
    int status;
    int resolver_threads = 0;;   
    for (i = 0; i < MAX_RESOLVER_THREADS; i++)
    {
      status = pthread_create(&(resolver_pool[i]), NULL, process_threads, outputfp);
      if (status != 0)
      {
        fprintf(stderr, "Error: fail to create resolver thread[%d].\n", i);
        // If there are less than MIN_RESOLVER_THREADS allow then exit program.
        if (resolver_threads < MIN_RESOLVER_THREADS)
        {
          fprintf(stderr, "Error: resolver threads are less than allowable, must exit!\n");
          return EXIT_FAILURE;
        }
      }
      else
      {
        resolver_threads++; // Success, keep count on resolver threads.
        if(DEBUG){
    ...
    {      
      ...
      if(!inputfp)
      {
        ...
        perror(errorstr);
        error_counter++;
        // Cannot open any files; none opened so need to close.
        if (error_counter == num_of_files)
        {
          fprintf(stderr, "Error: No more files to open! Exiting!\n");
          return EXIT_FAILURE;
        }
        else
        {
      ...
      else
      {
        ...
        {
          fprintf(stderr, "Error: No existing files to parse! Exiting!\n");
          return EXIT_FAILURE;
        }
    ...
    int close_status = 0;
    // Close output file
    close_status = fclose(outputfp);
    if (close_status != 0)
    {
      fprintf(stderr, "Failed to close output file...there may be a leak!\n");
    }
 ...
 *
 * @param inputfp A pointer to the input file descriptor.
 * @return Nothing
 **/
void* insert_to_queue(void* inputfp)
{
  ...
  while (fscanf((FILE*)inputfp, INPUTFS, domain_name) > 0)
  {
    ...
    {
      fprintf(stderr, "Error: failed to push \"%s\" into the request_queue.\n",
          domain_name);
    }
 ...
 * condition may occur.
 * 
 * @param p_outputfp A pointer to a void.
 * @return Nothing 
 */
