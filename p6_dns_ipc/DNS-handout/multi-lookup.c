...
#define DEBUG 0 // Set to one if you want to debug (main)
#define DEBUG_CONS 1 // To debug the consumers function
#define DEBUG_PRO 1 // To dubug the producers function
...
int main(int argc, char* argv[]){
    ...
    for(i=1; i<(argc-1); i++)
    {
        ...
        // Signal just in case one child is hung up. Need to iterate 
        // child_count if you have more than 2 children.
        ...
        {            
          ...
          if (pid < 0){
            fprintf(stderr,"Error:: wait returned an error [pid: <%d>]", pid);
    ...
    pthread_mutex_destroy(&(buffer_ptr->flag_mutex));
    
    // Free resources
    munmap(shared_memory, sizeof(buffer_t));
    return EXIT_SUCCESS;
}
...
void consume(int cons_rank, void* p_outputfp)
{
  ...
  { 
    ...
    char domain_name[sizeof(char)*SBUFSIZE];
    ...
    strcpy(domain_name, buffer_ptr->data[buffer_ptr->out]);
...
void produce(void* p_inputfp)
{
  ...
  while(1/*fscanf((FILE*)p_inputfp, INPUTFS, domain_name) > 0*/)
  { 
    char* domain_name;
    domain_name = (char*) malloc(sizeof(char)*SBUFSIZE);

    if(fscanf((FILE*)p_inputfp, INPUTFS, domain_name) <= 0)
    {
      free(domain_name);
      break;  
    }
