...
#define DEBUG 1 // Set to one if you want to debug.
#define DEBUG_CONS 1
#define DEBUG_PRO 0
...
int main(int argc, char* argv[]){
    ...
    int i;
    int files = 0;
    // Start the producers count at 0.
    producers = 0;
    ...
    pthread_mutexattr_setpshared(&(buffer_ptr->mutexAttr), PTHREAD_PROCESS_SHARED);
    ...
    for(i=1; i<(argc-1); i++)
    {
        ...
        int n = MIN_RESOLVER_PROCESSES;
        static int child_rank = 0;
        int parent = getpid();
        printf("Parent id before forck is <%d>\n", parent);
...
void* create_shared_memory(size_t size)
{
  ...
  // and stretch the file. We need write a blank space at the end
  ...
  *******************************************************************/
  // Need to be able to readna and write to shared memory
    ...
    int child_count)
{
  ...
  if (nprocess > 0)
  {
    ...
    else  // Parent (PRODUCER)
    {
      ...
      produce(p_inputfp); // Start producing
...
void consume(int cons_rank, void* p_outputfp)
{
  int buffer_count;
  int producers_done;
  // try to obtain the flag mutex lock
  ...
  if (DEBUG_CONS){
    ...
    printf("DEBUG::consume(): pointer in count is <%d>\n", buffer_ptr->in);
    printf("DEBUG::consume(): buffer count is <%d>\n", buffer_ptr->count);
    printf("DEBUG::consume(): flag producers_done is set to <%d>\n",
      buffer_ptr->producers_done);
    printf("-------ENTERING WHILE LOOP=================\n");
  }
 
  // So that consumers will have the chance to consume all
  while((buffer_count != 0) || !producers_done) 
  { 
    // try to obtain the buffer mutex lock
    pthread_mutex_lock(&(buffer_ptr->buffer_mutex));

    printf("DEBUG::consume() pointer in <%d>\n", buffer_ptr->in);
    printf("DEBUG::consume() pointer out <%d>\n", buffer_ptr->out);
    ...
    while (buffer_ptr->count == 0)
    {
      ...
      if (buffer_ptr->producers_done && (buffer_ptr->count == 0))
      {
        if (DEBUG_CONS){
          ...
          printf("DEBUG::consume(): Consumer#%d child [pid: <%d>] done consuming. Exiting!\n",
             cons_rank, child_pid);
        }
        // Release mutex lock and exit gracefully.
        pthread_mutex_unlock(&(buffer_ptr->buffer_mutex));
        free(domain_name);
        return;
      }
      ...
      if (DEBUG_CONS){
          int child_pid = getpid();
          printf("DEBUG::consume(): Consumer#%d, child [pid: <%d>] waiting to consume...\n",
            cons_rank, child_pid);
      }
    ...
    buffer_ptr->count--;
    // Signal the other consumers they can begin to get the mutex lock.
...
{
  char* domain_name;
  // Dynamically allocate memory for domain name
  domain_name = (char*) malloc(sizeof(char)*SBUFSIZE);
  ...
  {   
    ...
    // While bounded buffer is full, we must wait for condition variable
    while ( buffer_ptr->count == BUFFER_SIZE)
    {
      if (DEBUG_PRO){
        printf("DEBUG::produce(): Producer, [pid: <%d>] waiting to produce...\n",
          getpid());
      }
      pthread_cond_wait(&(buffer_ptr->buffer_empty), &(buffer_ptr->buffer_mutex));
    }
    //buffer_ptr->data[buffer_ptr->in] = (char**)  malloc(sizeof(char)*SBUFSIZE);
